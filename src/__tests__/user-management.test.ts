...
describe('User Management - Create User', () => {
  test('should create a new user successfully', async () => {
    ...
    if (result.ok) {
      ...
      expect(result.value.role).toBe('PRODUCTION');
...
describe('User Management - Permission Matrix', () => {
  ...
  test('Office role should have read-only user permissions', async () => {
    const { hasPermission } = await import('@/domain/models');

    expect(hasPermission('OFFICE', 'user:read')).toBe(true);
    expect(hasPermission('OFFICE', 'user:create')).toBe(false);
    expect(hasPermission('OFFICE', 'user:update')).toBe(false);
    expect(hasPermission('OFFICE', 'user:delete')).toBe(false);
  });
...
describe('User Management - Role Hierarchy', () => {
  test('Admin should be higher than all other roles', async () => {
    const { ROLE_HIERARCHY } = await import('@/domain/models');

    expect(ROLE_HIERARCHY.ADMIN).toBeGreaterThan(ROLE_HIERARCHY.OFFICE);
  ...
  test('isRoleAtLeast should correctly compare roles', async () => {
    ...
    expect(isRoleAtLeast('ADMIN', 'PRODUCTION')).toBe(true);
    expect(isRoleAtLeast('OFFICE', 'PRODUCTION')).toBe(true);
    expect(isRoleAtLeast('PRODUCTION', 'ADMIN')).toBe(false);
    expect(isRoleAtLeast('SALES', 'OFFICE')).toBe(false);
  });
});
