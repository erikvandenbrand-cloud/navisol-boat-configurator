...
describe('CE Chapter Scaffold', () => {
  test('CE scaffold has exactly 10 chapters (unified dossier sections)', () => {
    expect(CE_CHAPTER_SCAFFOLD.length).toBe(10);
  });

  test('CE scaffold chapters use section IDs as chapter numbers', () => {
    const expectedIds = [
      'general-description',
      'design-drawings',
      'calculations',
    ...
    for (let i = 0; i < CE_CHAPTER_SCAFFOLD.length; i++) {
      expect(CE_CHAPTER_SCAFFOLD[i].chapterNumber).toBe(expectedIds[i]);
    }
  });
  ...
  test('CE scaffold includes key chapters', () => {
    ...
    expect(titles).toContain('Stability & Buoyancy Assessment');
    expect(titles).toContain('Electrical Systems');
    expect(titles).toContain('Conformity Assessment Documentation');
  });
});

describe('ES-TRIN Chapter Scaffold', () => {
  test('ES-TRIN scaffold has 10 chapters (unified dossier sections)', () => {
    expect(ES_TRIN_CHAPTER_SCAFFOLD.length).toBe(10);
  });

  test('ES-TRIN scaffold includes key chapters', () => {
    const titles = ES_TRIN_CHAPTER_SCAFFOLD.map(c => c.title);
    expect(titles).toContain('General Description');
    expect(titles).toContain('Materials & Components');
    expect(titles).toContain('Fuel Systems');
  });
});

describe('Lloyds Chapter Scaffold', () => {
  test('Lloyds scaffold has 10 chapters (unified dossier sections)', () => {
    expect(LLOYDS_CHAPTER_SCAFFOLD.length).toBe(10);
  });

  test('Lloyds scaffold includes key chapters', () => {
    const titles = LLOYDS_CHAPTER_SCAFFOLD.map(c => c.title);
    expect(titles).toContain('Structural & Engineering Calculations');
    expect(titles).toContain('Design Drawings & Plans');
  });
});
...
describe('getChapterScaffold', () => {
  test('returns CE scaffold for CE type', () => {
    ...
    expect(scaffold.length).toBe(10);
  });
  ...
  test('returns unified scaffold for OTHER type', () => {
    // All certification types now use the same 10 dossier sections
...
describe('Import/Export Compatibility', () => {
  ...
  test('compliance packs survive stringify-parse roundtrip', () => {
    ...
    expect(parsed.chapters.length).toBe(10);
...
describe('CE Checklist Scaffold', () => {
  test('CE checklist scaffold covers all 10 dossier sections', () => {
    const sectionIds = [
      'general-description',
      'design-drawings',
      'calculations',
    ...
    for (const sectionId of sectionIds) {
      const sectionItems = CE_CHECKLIST_SCAFFOLD[sectionId];
      expect(sectionItems).toBeDefined();
      expect(sectionItems.length).toBeGreaterThan(0);
    }
  });

  test('each checklist item has required fields', () => {
    for (const [sectionId, items] of Object.entries(CE_CHECKLIST_SCAFFOLD)) {
  ...
  test('General Description section has identification items', () => {
    const generalDesc = CE_CHECKLIST_SCAFFOLD['general-description'];
    const titles = generalDesc.map(i => i.title);
    expect(titles.some(t => t.includes('WIN') || t.includes('identification'))).toBe(true);
  });

  test('Conformity Assessment section has mandatory DoC item', () => {
    const conformity = CE_CHECKLIST_SCAFFOLD['conformity-assessment'];
    const docItem = conformity.find(i => i.title.includes('Declaration of Conformity'));
    expect(docItem).toBeDefined();
    expect(docItem?.mandatory).toBe(true);
  });

  test('Essential Requirements section has optional items (gas system)', () => {
    const essentialReq = CE_CHECKLIST_SCAFFOLD['essential-requirements'];
    const optionalItems = essentialReq.filter(i => !i.mandatory);
...
describe('getChapterChecklist', () => {
  test('returns CE checklist for CE type', () => {
    const checklist = getChapterChecklist('CE', 'general-description');
    expect(checklist.length).toBeGreaterThan(0);
  });

  test('returns empty array for non-existent chapter', () => {
    const checklist = getChapterChecklist('CE', 'non-existent-section');
