...
export const ProductionProcedureService = {
  ...
  async isTaskSetAlreadyApplied(
    ...
    // Check if any production work item has this sourceTaskSetTemplateId
    return (project.workItems || []).some(
      (w) => w.kind === 'production' && w.sourceTaskSetTemplateId === taskSetId
    );
  },
  ...
  /**
   * Initialize default production procedures - only runs once, even if data is deleted
   */
  async initializeDefaults(context: AuditContext): Promise<void> {
    // Check the persistent initialization flag
    const alreadyInitialized = await SettingsService.hasInitialized('defaultProcedures');
    if (alreadyInitialized) return;

    // Check if data exists (for backward compatibility)
    const existing = await ProductionProcedureRepository.getAll();
    if (existing.length > 0) {
      await SettingsService.markInitialized('defaultProcedures');
      return;
    }
