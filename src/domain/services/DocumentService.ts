...
import { SettingsService, type CompanyInfo } from './SettingsService';
...
/**
 * Standard company info object for document rendering.
 * Resolved from Settings - single source of truth for builder identity.
 */
...
/**
 * Convert SettingsService CompanyInfo to document format
 */
function formatCompanyInfo(info: CompanyInfo): DocumentCompanyInfo {
  return {
    name: info.legalName || info.name,
    address: `${info.street}, ${info.postalCode} ${info.city}`,
    city: info.city,
    phone: info.phone,
    email: info.email,
    vat: info.vatNumber,
    kvk: info.chamberOfCommerce,
  };
}

/**
 * Check if company info is complete for document generation.
 * Returns list of missing fields if incomplete.
 */
function validateCompanyInfo(info: CompanyInfo): { valid: boolean; missing: string[] } {
  const missing: string[] = [];
  if (!info.name && !info.legalName) missing.push('Company Name');
  if (!info.street) missing.push('Street Address');
...
/**
 * Result type for document generation that includes optional warnings.
 */
...
/**
 * Resolved compliance values for document generation.
 * Uses project override if set, else falls back to model default.
 */
export interface ResolvedComplianceValues {
  effectiveRcdModule?: string;
  effectiveNotifiedBody?: string;
  effectiveNotifiedBodyName?: string;
  source: 'project' | 'model' | 'none';
}

/**
 * Resolve effective RCD Module and Notified Body values.
 * Priority: project.declarations override > boatModel.specifications.compliance default
 */
export function resolveComplianceValues(
  project: Project,
  boatModel?: { specifications?: { compliance?: { rcdModuleApplied?: string; notifiedBodyNumber?: string } } } | null
): ResolvedComplianceValues {
  const projectRcdModule = project.declarations?.conformityModule;
  const projectNotifiedBody = project.declarations?.notifiedBodyNumber;
  const projectNotifiedBodyName = project.declarations?.notifiedBodyName;
  ...
  // Determine source
  const hasProjectOverride = !!(projectRcdModule || projectNotifiedBody);
  const hasModelDefault = !!(modelRcdModule || modelNotifiedBody);
  const source: 'project' | 'model' | 'none' =
    hasProjectOverride ? 'project' :
    hasModelDefault ? 'model' : 'none';

  return {
    effectiveRcdModule: projectRcdModule || modelRcdModule,
    effectiveNotifiedBody: projectNotifiedBody || modelNotifiedBody,
    effectiveNotifiedBodyName: projectNotifiedBodyName,
    source,
  };
}
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          ...
          <p>${data.companyName} • ${data.companyAddress} • ${data.companyPhone}</p>
        </div>
      </div>
    </body>
    </html>
...
function generateOwnersManualHtml(project: Project, client: Client, companyInfo: DocumentCompanyInfo): string {
  return `
    <!DOCTYPE html>
    <html>
    <head>
    ...
    <body>
      ...
      <div class="container">
        ...
        <p>Welcome to your new ${companyInfo.name}. This manual provides essential information for the safe operation and maintenance of your vessel.</p>
...
function generateCeDeclarationHtml(
  project: Project,
  client: Client,
  companyInfo: DocumentCompanyInfo,
  complianceValues: ResolvedComplianceValues
): string {
  // Build conformity assessment section based on effective values
  const rcdModuleText = complianceValues.effectiveRcdModule
    ? `Module ${complianceValues.effectiveRcdModule}`
    : 'Not specified';

  const notifiedBodyText = complianceValues.effectiveNotifiedBody
    ? `Notified Body No. ${complianceValues.effectiveNotifiedBody}${complianceValues.effectiveNotifiedBodyName ? ` (${complianceValues.effectiveNotifiedBodyName})` : ''}`
    : 'Not applicable (internal production control)';

  // Signatory info from declarations
  const signatory = project.declarations?.docSignatory || companyInfo.name;
  const signatoryRole = project.declarations?.docSignatoryRole || 'Authorized Representative';
  const docReference = project.declarations?.docReferenceNumber || `DOC-${project.projectNumber}`;
  const issueDate = project.declarations?.docIssueDate
    ? new Date(project.declarations.docIssueDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' })
    ...
    <body>
      <div class="container">
        ...
        <div class="section">
          ...
          ${project.vesselIdentity?.win ? `<p><strong>WIN:</strong> ${project.vesselIdentity.win}</p>` : ''}
        </div>
        ...
        <div class="section">
          ...
          <p><strong>Assessment Module:</strong> ${rcdModuleText}</p>
          <p><strong>Notified Body:</strong> ${notifiedBodyText}</p>
        </div>
        ...
        <div class="signature">
          <div class="signature-box">
            ...
            <p style="margin-top: 8px;">${companyInfo.city}, ${issueDate}</p>
          </div>
          <div class="signature-box">
            <div class="signature-line">Authorized Signatory</div>
            <p style="margin-top: 8px;">${signatory}</p>
            <p style="font-size: 9pt; color: #64748b;">${signatoryRole}</p>
          </div>
        </div>
...
function generateTechnicalFileHtml(project: Project, client: Client, companyInfo: DocumentCompanyInfo): string {
      ...
      <div class="container">
        ...
        <div class="section">
          ...
          <table>
            ...
            <tr><th>Project Type</th><td>${project.type.replace('_', ' ')}</td></tr>
            <tr><th>Owner</th><td>${client.name}</td></tr>
            <tr><th>Manufacturer</th><td>${companyInfo.name}</td></tr>
        ...
        <div class="section">
          ...
          <p>${companyInfo.name} declares that this technical file contains all information required by the Recreational Craft Directive 2013/53/EU and that the vessel described herein meets the essential requirements.</p>
...
export const DocumentService = {
   ...
   * Returns warnings if company info is incomplete.
   */
  async generateOwnersManual(
    projectId: string
  ): Promise<Result<DocumentGenerationResult, string>> {
    const project = await ProjectRepository.getById(projectId);
    if (!project) {
      return Err('Project not found');
    }
    ...
    const rawCompanyInfo = await SettingsService.getCompanyInfo();
    const validation = validateCompanyInfo(rawCompanyInfo);
    const companyInfo = formatCompanyInfo(rawCompanyInfo);

    const warnings: string[] = [];
    if (!validation.valid) {
      warnings.push(`Company info incomplete. Missing: ${validation.missing.join(', ')}. Configure in Settings → Company.`);
    }

    return Ok({
      html: generateOwnersManualHtml(project, client, companyInfo),
      warnings,
    });
  },
   ...
   * Returns warnings if company info is incomplete or RCD module/notified body are missing.
   */
  async generateCeDeclaration(
    ...
    // Fetch boat model for compliance defaults
    let boatModel = null;
    if (project.configuration.boatModelVersionId) {
      try {
        ...
        console.warn('[DocumentService] Failed to load boat model for CE Declaration:', e);
      }
    }

    // Resolve effective compliance values (project override > model default)
    const complianceValues = resolveComplianceValues(project, boatModel);

    // Warn if RCD module is missing (required for compliance)
    if (!complianceValues.effectiveRcdModule) {
      warnings.push('RCD conformity module not specified. Set in Boat Model → Technical Specifications → Compliance, or use project override in Declarations.');
    }

    return Ok({
      html: generateCeDeclarationHtml(project, client, companyInfo, complianceValues),
      warnings,
    });
  },
  ...
  async generateTechnicalFile(
    ...
    return Ok({
      html: generateTechnicalFileHtml(project, client, companyInfo),
      warnings,
    });
  },
   ...
   * Builder identity is always sourced from Settings, not from project data.
   */
  async generateFromTemplate(
    ...
    // Get company info from Settings for builder identity
    // This is the single source of truth for builder name/address in documents
    const settingsCompanyInfo = await SettingsService.getCompanyInfo();
    const companyValidation = validateCompanyInfo(settingsCompanyInfo);
    const companyInfo = {
      name: settingsCompanyInfo.legalName || settingsCompanyInfo.name,
      address: `${settingsCompanyInfo.street}, ${settingsCompanyInfo.postalCode} ${settingsCompanyInfo.city}`,
      phone: settingsCompanyInfo.phone,
      email: settingsCompanyInfo.email,
      vat: settingsCompanyInfo.vatNumber,
      kvk: settingsCompanyInfo.chamberOfCommerce,
      city: settingsCompanyInfo.city,
    };

    // Warn if company info is incomplete (but still generate document)
    if (!companyValidation.valid) {
      console.warn(`[DocumentService] Company info incomplete for ${type}. Missing: ${companyValidation.missing.join(', ')}`);
    }
    ...
    if (templateData) {
      ...
      htmlContent = TemplateService.renderTemplate(defaultContent, project, client, companyInfo);
