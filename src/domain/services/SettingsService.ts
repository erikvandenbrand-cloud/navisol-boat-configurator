...
/**
 * Tracks what data has been initialized/seeded
 * Once a flag is set to true, it stays true even if data is deleted
 */
export interface InitializationFlags {
  libraryTaxonomy?: boolean;
  sampleArticles?: boolean;
  sampleWorkInstructions?: boolean;
  defaultTemplates?: boolean;
  defaultProcedures?: boolean;
  defaultBoatModels?: boolean;
  defaultCatalogs?: boolean;
  offerTemplates?: boolean;
  standardsLibrary?: boolean;
  documentTemplates?: boolean;
  defaultSuppliers?: boolean;
}
...
export interface AppSettings {
  ...
  /** Tracks what data has been initialized - prevents re-seeding after deletion */
...
const DEFAULT_INITIALIZATION_FLAGS: InitializationFlags = {
  libraryTaxonomy: false,
  sampleArticles: false,
  sampleWorkInstructions: false,
  defaultTemplates: false,
  defaultProcedures: false,
  defaultBoatModels: false,
  defaultCatalogs: false,
  offerTemplates: false,
  standardsLibrary: false,
  documentTemplates: false,
  defaultSuppliers: false,
};
...
export const SettingsService = {
  ...
  async initializeDefaults(): Promise<AppSettings> {
    const settings: AppSettings = {
      ...
      defaultCurrency: 'EUR',
      initializationFlags: DEFAULT_INITIALIZATION_FLAGS,
      createdAt: now(),
      updatedAt: now(),
    };
  ...
  /**
   * Check if a specific initialization has been done
   */
  async hasInitialized(flag: keyof InitializationFlags): Promise<boolean> {
    const settings = await this.getSettings();
    return settings.initializationFlags?.[flag] === true;
  },

  /**
   * Mark an initialization as complete
   * Once marked, the flag persists even if data is deleted
   */
  async markInitialized(flag: keyof InitializationFlags): Promise<void> {
    const settings = await this.getSettings();

    const currentFlags = settings.initializationFlags || DEFAULT_INITIALIZATION_FLAGS;
  ...
  /**
   * Get all initialization flags
   */
  async getInitializationFlags(): Promise<InitializationFlags> {
    const settings = await this.getSettings();
    return settings.initializationFlags || DEFAULT_INITIALIZATION_FLAGS;
  },

  /**
   * Reset an initialization flag (allows re-seeding)
   * Use with caution - this will allow data to be re-seeded on next load
   */
  async resetInitializationFlag(flag: keyof InitializationFlags): Promise<void> {
    const settings = await this.getSettings();
