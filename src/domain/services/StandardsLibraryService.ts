...
export const StandardsLibraryService = {
  ...
  /**
   * Initialize the library with seed data - only runs once, even if data is deleted
   */
  async initializeSeedData(): Promise<void> {
    // Check the persistent initialization flag
    const alreadyInitialized = await SettingsService.hasInitialized('standardsLibrary');
    if (alreadyInitialized) return;

    // Check if data exists (for backward compatibility)
    const count = await StandardsLibraryRepository.count();
    if (count > 0) {
      await SettingsService.markInitialized('standardsLibrary');
      return;
    }
