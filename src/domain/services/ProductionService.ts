...
export const ProductionService = {
  ...
  /**
   * Get production work items for a stage (using stageId field)
   * Now uses unified workItems array with kind: 'production'
   */
  async getTasksForStage(projectId: string, stageId: string): Promise<WorkItem[]> {
    const project = await ProjectRepository.getById(projectId);
    if (!project) {
      return [];
    }

    // Filter workItems by kind: 'production' and stageId
    return (project.workItems || []).filter(
      (w) => w.kind === 'production' && w.stageId === stageId
    );
  },
  ...
  async toggleAutoProgress(
    ...
    // Calculate progress from production work items if enabling auto-progress
    let newProgress = stage.progressPercent;
    if (enabled) {
      const tasks = (project.workItems || []).filter(
        (w) => w.kind === 'production' && w.stageId === stageId
      );
  ...
  async recalculateStageProgress(
    ...
    // Use production work items from unified workItems array
    const tasks = (project.workItems || []).filter(
      (w) => w.kind === 'production' && w.stageId === stageId
    );
    const completed = tasks.filter((t) => t.status === 'COMPLETED').length;
