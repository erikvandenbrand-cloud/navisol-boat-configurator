...
export const AuthService = {
  ...
  async createUser(
    ...
    const user: User = {
      ...
      isActive: true,
      aiEnabled: false, // AI disabled by default - explicit admin action required to enable
      phone: input.phone,
  ...
  async initializeDefaultUser(): Promise<void> {
    ...
    // Admin must create first user manually via Settings > Users
    ...
    // Note: aiEnabled defaults to false - must be explicitly enabled by admin
    ...
    // GOVERNANCE: All users have aiEnabled: false by default
    // AI can only be enabled by an explicit admin action
    // Roles (v320): ADMIN, OFFICE, SALES, PRODUCTION
    const demoUsers: User[] = [
      {
        id: 'user-office',
        email: 'office@eagleboats.nl',
        name: 'Peter de Vries',
        role: 'OFFICE',
        passwordHash: hashPassword('office123'),
        isActive: true,
        aiEnabled: false, // AI disabled by default - explicit admin action required
