...
export const LibraryArticleService = {
   ...
   */
  async getAll(includeArchived = false): Promise<LibraryArticle[]> {
  ...
  /**
   * Get non-archived articles by subcategory
   */
  async getBySubcategory(subcategoryId: string, includeArchived = false): Promise<LibraryArticle[]> {
    const all = await ArticleRepository.getBySubcategory(subcategoryId);
    if (includeArchived) return all;
    return all.filter(a => !a.archived);
  },

  /**
   * Search non-archived articles
   */
  async search(query: string, includeArchived = false): Promise<LibraryArticle[]> {
    const results = await ArticleRepository.search(query);
    if (includeArchived) return results;
    return results.filter(a => !a.archived);
  },
  ...
  async create(input: CreateArticleInput, context: AuditContext): Promise<Result<LibraryArticle, string>> {
    ...
    const article: LibraryArticle = {
      ...
      // Supplier cost for BOM defaults (optional)
      supplierCostExclVat: input.supplierCostExclVat ?? null,
      supplierCostSourceNote: input.supplierCostSourceNote ?? null,
      createdAt: now(),
      updatedAt: now(),
      version: 0,
    };
    ...
    await AuditService.logCreate(context, 'LibraryArticle', articleId, {
      code: input.code,
      name: input.name,
      supplierCostExclVat: input.supplierCostExclVat,
    });
  ...
  /**
   * Update the supplier cost on an existing article.
   * This is an article-level field, not version-level.
   * Does NOT retroactively change existing BOM lines.
   */
  async updateSupplierCost(
    articleId: string,
    supplierCostExclVat: number | null,
    supplierCostSourceNote: string | null,
    context: AuditContext
  ): Promise<Result<LibraryArticle, string>> {
    const article = await ArticleRepository.getById(articleId);
    if (!article) return Err('Article not found');
    ...
    await AuditService.log(
      ...
      'LibraryArticle',
      articleId,
      `Updated supplier cost for ${article.code}: ${supplierCostExclVat != null ? `€${supplierCostExclVat}` : 'cleared'}`
    );
  ...
  /**
   * Update article metadata (name, supplier, tags, etc.)
  ...
  async update(
    ...
    const updated: LibraryArticle = {
      ...article,
      name: updates.name ?? article.name,
      supplierId: updates.supplierId !== undefined ? (updates.supplierId || undefined) : article.supplierId,
      supplierName: updates.supplierName !== undefined ? (updates.supplierName || undefined) : article.supplierName,
      unit: updates.unit ?? article.unit,
    ...
    await AuditService.log(
      ...
      'LibraryArticle',
      articleId,
      `Updated article ${article.code}: ${Object.keys(updates).join(', ')}`
    );
   ...
   * Archived articles are hidden from normal lists but can be restored
   */
  async archive(articleId: string, context: AuditContext): Promise<Result<LibraryArticle, string>> {
    const article = await ArticleRepository.getById(articleId);
    if (!article) return Err('Article not found');
    if (article.archived) return Err('Article is already archived');
   ...
   */
  async restore(articleId: string, context: AuditContext): Promise<Result<LibraryArticle, string>> {
    const article = await ArticleRepository.getById(articleId);
    if (!article) return Err('Article not found');
    if (!article.archived) return Err('Article is not archived');
  ...
  /**
   * Permanently delete an article and all its versions
   * Use with caution - this cannot be undone
   */
  async delete(articleId: string, context: AuditContext): Promise<Result<void, string>> {
    const article = await ArticleRepository.getById(articleId);
    if (!article) return Err('Article not found');

    // Delete all versions first
    const versions = await ArticleVersionRepository.getByArticle(articleId);
    for (const version of versions) {
      await ArticleVersionRepository.delete(version.id);
    }
    ...
    await AuditService.log(
      ...
      'LibraryArticle',
      articleId,
      `Permanently deleted article ${article.code} and ${versions.length} versions`
    );
  ...
  /**
   * Get all articles including archived ones
   */
  async getAllIncludingArchived(): Promise<LibraryArticle[]> {
...
export const LibrarySeedService = {
  ...
  async initializeTaxonomy(context: AuditContext): Promise<void> {
    ...
    const alreadyInitialized = await SettingsService.hasInitialized('libraryTaxonomy');
    if (alreadyInitialized) {
      console.log('Library taxonomy already initialized (flag set)');
      return;
    }

    // Also check if data exists (for backward compatibility with existing installations)
    const existingCategories = await CategoryRepository.count();
    if (existingCategories > 0) {
      // Data exists but flag not set - set the flag and return
      await SettingsService.markInitialized('libraryTaxonomy');
      console.log('Library taxonomy exists - marked as initialized');
      return;
    }
  ...
  async seedSampleArticles(context: AuditContext): Promise<void> {
    // Check the persistent initialization flag
    const alreadyInitialized = await SettingsService.hasInitialized('sampleArticles');
    if (alreadyInitialized) {
      console.log('Sample articles already initialized (flag set)');
      return;
    }
    ...
    if (articles.length > 0) {
      // Data exists but flag not set - set the flag and return
      await SettingsService.markInitialized('sampleArticles');
      console.log('Sample articles exist - marked as initialized');
      return;
    }
