...
export const BOMService = {
  ...
  /**
   * Generate BOM from a Quote's configuration
   *
   ...
   * @param quoteId - The quote ID to generate BOM from (required)
   * @param context - Audit context
   * @param trigger - What triggered the BOM generation
   */
  async generateBOM(
    projectId: string,
    quoteId: string,
    ...
    // Use configuration snapshot from quote if available, otherwise latest or current
    // Note: Quotes may have a configuration snapshot reference in the future
    // For now, use the latest configuration snapshot or 'current'
    ...
    await AuditService.log(
      ...
      'BOMSnapshot',
      bomSnapshot.id,
      `Generated BOM Baseline #${snapshotNumber} from Quote ${quote.quoteNumber} v${quote.version} with ${totalParts} parts (${trigger}). ${estimatedCostCount} items estimated at ${Math.round(estimationRatio * 100)}%.`
    );
   ...
   *
   * Cost resolution rules (explicit priority):
   * 1. If Article.supplierCostExclVat is present → use it, source = "LIBRARY"
   * 2. Else if ArticleVersion.costPrice is present → use it, source = "LIBRARY" (version cost)
   * 3. Else → estimate from sell price, source = "ESTIMATED"
   *
   * Note: "OVERRIDE" is only set when user manually edits a BOM line after generation.
   */
  async expandArticleItem(
    ...
    // Get article for code and supplier cost
    ...
    // Determine cost source and value (explicit priority order)
    let unitCost: number;
    let costSource: 'LIBRARY' | 'ESTIMATED';
    let librarySupplierCost: number | undefined;
    let librarySupplierCostNote: string | undefined;

    // 1. Check Article.supplierCostExclVat first (library-level supplier cost)
    if (article?.supplierCostExclVat != null && article.supplierCostExclVat > 0) {
      unitCost = article.supplierCostExclVat;
      costSource = 'LIBRARY';
      librarySupplierCost = article.supplierCostExclVat;
      librarySupplierCostNote = article.supplierCostSourceNote ?? undefined;
    }
    // 2. Fall back to ArticleVersion.costPrice (version-level cost)
    else if (articleVersion.costPrice != null && articleVersion.costPrice > 0) {
      unitCost = articleVersion.costPrice;
      costSource = 'LIBRARY';
    }
    // 3. Estimate from sell price
    else {
      unitCost = Math.round(articleVersion.sellPrice * estimationRatio);
      costSource = 'ESTIMATED';
    }

    const totalCost = quantity * unitCost;
    const isEstimated = costSource === 'ESTIMATED';
    ...
    if (itemMap.has(key)) {
      ...
      const bomItem: BOMItem = {
        ...
        costSource,
        // Legacy estimation tracking (kept for backward compatibility)
        isEstimated,
        estimationRatio: isEstimated ? estimationRatio : undefined,
        sellPrice: isEstimated ? articleVersion.sellPrice : undefined,
        // Library supplier cost info (for tooltip)
        librarySupplierCost,
        librarySupplierCostNote,
   ...
   *
   * Cost resolution follows same rules as expandArticleItem for each component.
   */
  async expandKitItem(
    configItem: ConfigurationItem,
    itemMap: Map<string, BOMItem>,
    ...
    if (!kitVersion.explodeInBOM || kitVersion.salesOnly) {
      ...
      const totalCost = quantity * unitCost;
      const isEstimated = !hasActualCost && !cost;
      ...
      if (itemMap.has(key)) {
        ...
        const bomItem: BOMItem = {
          ...
          totalCost,
          // Cost source
          costSource: isEstimated ? 'ESTIMATED' : 'LIBRARY',
          // Estimation tracking
          isEstimated,
          estimationRatio: isEstimated ? estimationRatio : undefined,
          sellPrice: isEstimated ? kitVersion.sellPrice : undefined,
        };
        itemMap.set(key, bomItem);
      }
      return;
    ...
    for (const component of kitVersion.components) {
      ...
      // Determine cost source and value (same rules as expandArticleItem)
      let unitCost: number;
      let costSource: 'LIBRARY' | 'ESTIMATED';
      let librarySupplierCost: number | undefined;
      let librarySupplierCostNote: string | undefined;

      // 1. Check Article.supplierCostExclVat first
      if (article.supplierCostExclVat != null && article.supplierCostExclVat > 0) {
        unitCost = article.supplierCostExclVat;
        costSource = 'LIBRARY';
        librarySupplierCost = article.supplierCostExclVat;
      ...
      // 2. Fall back to ArticleVersion.costPrice
      else if (articleVersion.costPrice != null && articleVersion.costPrice > 0) {
        unitCost = articleVersion.costPrice;
        costSource = 'LIBRARY';
      }
   ...
   * Custom items always have costSource = 'ESTIMATED' since there's no library entry.
   */
  expandCustomItem(
    configItem: ConfigurationItem,
    itemMap: Map<string, BOMItem>,
  ...
  /**
   * Export BOM to CSV format with cost source tracking
   */
  exportToCSV(bom: BOMSnapshot): string {
    ...
    const rows = bom.items.map(item => [
      ...
      item.costSource || (item.isEstimated ? 'ESTIMATED' : 'LIBRARY'),
      ...
      item.librarySupplierCost ? String(item.librarySupplierCost) : '',
      item.librarySupplierCostNote || '',
      item.supplier || '',
