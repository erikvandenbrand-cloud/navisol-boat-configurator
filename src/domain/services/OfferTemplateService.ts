...
export const OfferTemplateService = {
  ...
  /**
   * Initialize seed data - only runs once, even if data is deleted
   */
  async initializeSeedData(): Promise<void> {
    // Check the persistent initialization flag
    const alreadyInitialized = await SettingsService.hasInitialized('offerTemplates');
    if (alreadyInitialized) return;

    // Check if data exists (for backward compatibility)
    const count = await OfferTemplateRepository.count();
    if (count > 0) {
      await SettingsService.markInitialized('offerTemplates');
      return;
    }
