...
export const EquipmentCatalogService = {
  ...
  /**
   * Initialize default equipment - only runs once, even if data is deleted
   */
  async initializeDefaults(context: AuditContext): Promise<void> {
    // Check the persistent initialization flag
    const alreadyInitialized = await SettingsService.hasInitialized('defaultCatalogs');
    if (alreadyInitialized) return;

    // Check if data exists (for backward compatibility)
    const count = await EquipmentRepository.count();
    if (count > 0) {
      await SettingsService.markInitialized('defaultCatalogs');
      return;
    }
