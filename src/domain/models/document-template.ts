...
| Watercraft Identification Number (WIN) | {{WIN}} |
...
function getDefaultRequiredFields(type: CEDocumentTemplateType): string[] {
  switch (type) {
    case 'DOC_DOC':
      return ['MANUFACTURER_NAME', 'BOAT_MODEL', 'WIN', 'DESIGN_CATEGORY', 'ISSUE_DATE', 'SIGNATORY_NAME'];
    case 'DOC_OWNERS_MANUAL':
      return ['BOAT_MODEL', 'WIN', 'MANUFACTURER_NAME'];
    case 'DOC_CE_MARKING_CERT':
      return ['MANUFACTURER_NAME', 'BOAT_MODEL', 'WIN', 'ISSUE_DATE', 'CONFORMITY_MODULE'];
    case 'DOC_ANNEX_INDEX':
      return ['PROJECT_TITLE', 'PROJECT_NUMBER', 'BOAT_MODEL', 'WIN'];
    default:
      return [];
  }
}
 ...
 * For REFIT/MAINTENANCE/B2B_AFBOUW: returns empty array (templates are optional).
 */
export function ensureDocumentTemplates(
  projectType: 'NEW_BUILD' | 'REFIT' | 'MAINTENANCE' | 'B2B_AFBOUW',
  projectId: string,
  existingTemplates: ProjectDocumentTemplate[] | undefined,
  userId: string,
  projectSystems?: string[]
): ProjectDocumentTemplate[] {
  // REFIT/MAINTENANCE/B2B_AFBOUW projects don't require templates
