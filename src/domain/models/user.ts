...
/**
 * Visibility Roles (v320 plan)
 * - ADMIN: Full system access. Manages staff, settings, library.
 * - OFFICE: Full project access. Creates projects, manages compliance, quotes, BOM.
 * - PRODUCTION: Portfolio execution only. Tasks, timesheets, shop floor orders.
 * - SALES: Read-only projects + quote editing. No production, no compliance editing.
 *
 * Note: This is visibility-only enforcement. Real route guards may be added later.
 */
export type UserRole = 'ADMIN' | 'OFFICE' | 'SALES' | 'PRODUCTION';
...
export interface User extends Entity {
  ...
  department?: string;
  /**
   * Whether AI assistance features are enabled for this user.
   * When false, AI "Suggest" buttons are hidden throughout the UI.
   * This is a user-level opt-in, not role-based.
   */
  aiEnabled?: boolean;
  createdAt: string;
  updatedAt: string;
...
export interface CreateUserInput {
  ...
  phone?: string;
  department?: string;
  /** Enable AI assistance features for this user */
  aiEnabled?: boolean;
}
...
export interface UpdateUserInput {
  ...
  department?: string;
  isActive?: boolean;
  /** Enable/disable AI assistance features */
  aiEnabled?: boolean;
}
