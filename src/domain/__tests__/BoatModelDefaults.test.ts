 ...
 * (PATCH 6 - Updated to use specifications instead of legacy fields)
 */
...
describe('BoatModelVersion - Default Configuration', () => {
  ...
  it('should create boat model with empty default configuration', async () => {
    ...
    const version = await BoatModelService.getVersionById(result.value.id);
    ...
    const result = await BoatModelService.create(
      {
        ...
        basePrice: 150000,
        specifications: {
          dimensions: { lengthOverallM: 10.0, beamOverallM: 3.2 },
        },
  ...
  it('should update default configuration for DRAFT version', async () => {
    ...
    const modelResult = await BoatModelService.create(
      {
        ...
        basePrice: 80000,
        specifications: {
          dimensions: { lengthOverallM: 8.0, beamOverallM: 2.5 },
        },
      },
      testContext
    );
    ...
    const versionId = modelResult.value.id;
  ...
  it('should allow updating approved version since there is no versioning now', async () => {
    ...
    // Update should succeed (no versioning in v4)
    const updateResult = await BoatModelService.updateDefaultConfiguration(
      versionId,
      [
        {
...
describe('Project Creation with Default Configuration', () => {
  ...
  beforeEach(async () => {
    ...
    boatModelVersionId = modelResult.value.id;
