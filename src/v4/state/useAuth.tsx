...
export interface CurrentUser {
   ...
   * UI elements for AI suggestions should only render when this is true.
   */
  aiEnabled: boolean;
}
...
interface AuthContextValue {
  ...
  /**
   * Check if AI assistance features are available for the current user.
   * Returns false if user is not authenticated or aiEnabled is false.
   */
  canUseAI: () => boolean;
  // Helpers
...
export function AuthProvider({ children }: { children: ReactNode }) {
        ...
        if (validation.valid && validation.user) {
          ...
          if (currentSession) {
            ...
            setUser({
              ...
              aiEnabled: validation.user.aiEnabled ?? false,
            });
          }
        } else {
  ...
  const login = useCallback(async (email: string, password: string) => {
    ...
    if (result.ok) {
      ...
      // Fetch full user data to get aiEnabled
      const fullUser = await AuthService.getUserById(newSession.userId);
      setUser({
        id: newSession.userId,
        name: newSession.name,
        email: newSession.email,
        role: newSession.role,
        aiEnabled: fullUser?.aiEnabled ?? false,
      });
      return { success: true };
    }
