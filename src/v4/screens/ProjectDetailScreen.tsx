...
import type { Project, Client, ProjectQuote, ConfigurationItem, ProjectStatus, ProjectType, AddConfigurationItemInput, AmendmentType, ProjectAmendment, ProjectTask, CreateTaskInput, TaskStatus, WorkItem } from '@/domain/models';
import { ProjectRepository, ClientRepository } from '@/data/repositories';
import { ProjectService, QuoteService, ConfigurationService, DocumentService, AmendmentService, BOMService, TaskService, DeliveryPackService, EquipmentListService, TemplateService, openPDFWindow } from '@/domain/services';
...
import { ProjectStageKanbanTab } from '@/v4/components/ProjectStageKanbanTab';
import { BoatsSection } from '@/v4/components/BoatsSection';
import { CEStatusWidget } from '@/v4/components/CEStatusWidget';
...
import { GenerateEquipmentListDialog } from '@/v4/components/GenerateEquipmentListDialog';
import { EquipmentListViewer } from '@/v4/components/EquipmentListViewer';
import { ShopFloorOrdersTab } from '@/v4/components/ShopFloorOrdersTab';
import { CustomerOfferPanel } from '@/v4/components/CustomerOfferPanel';
import {
  getDraftVersion,
  getApprovedVersion,
...
export function ProjectDetailScreen({ projectId, onBack, initialTab, dossierDeepLink }: ProjectDetailScreenProps) {
  ...
  const [showGenerateEquipmentListDialog, setShowGenerateEquipmentListDialog] = useState(false);
  const [viewingEquipmentList, setViewingEquipmentList] = useState<EquipmentListDocument | null>(null);

  // BOM generation state
  const [showGenerateBOMDialog, setShowGenerateBOMDialog] = useState(false);
  const [selectedBOMQuoteId, setSelectedBOMQuoteId] = useState<string>('');

  // Production view mode: 'list' or 'kanban'
  const [productionViewMode, setProductionViewMode] = useState<'list' | 'kanban'>('list');
  ...
  async function handleDownloadQuotePdf(quoteId: string) {
    ...
    // Use the template system for quote generation
    const quoteTemplate = TemplateService.getDefaultTemplate('QUOTE');
    const html = TemplateService.renderQuoteTemplate(
      quoteTemplate,
      {
  ...
  // Open BOM generation dialog (requires quote selection)
  function handleOpenGenerateBOMDialog() {
    if (!project) return;

    // If only one quote, pre-select it
    if (project.quotes.length === 1) {
      setSelectedBOMQuoteId(project.quotes[0].id);
    } else {
      setSelectedBOMQuoteId('');
    }
    setShowGenerateBOMDialog(true);
  }

  // Generate BOM from selected quote
  async function handleGenerateBOM() {
    if (!selectedBOMQuoteId) {
      alert('Please select a quote');
      return;
    }

    const result = await BOMService.generateBOM(projectId, selectedBOMQuoteId, getAuditContext(), 'MANUAL');

    if (result.ok) {
      setShowGenerateBOMDialog(false);
      await loadProject();
    } else {
  ...
  function handleEditTask(task: WorkItem) {
    setEditingTask(task);
    setShowTaskDialog(true);
  }
  ...
  async function handleUpdateTask(updates: Partial<WorkItem>) {
  ...
  // Helper: Get brand for equipment list
  function getEquipmentListBrand(brandKey: string): EquipmentListBrand {
    return DEFAULT_EQUIPMENT_LIST_BRANDS.find(b => b.key === brandKey)
      || DEFAULT_EQUIPMENT_LIST_BRANDS[0];
  }

  // Export Equipment List as PDF (opens in new window for print-to-PDF)
  function handleExportEquipmentListPDF(equipList: EquipmentListDocument) {
    if (!project) return;

    // Use the template system for equipment list generation
    const equipmentListTemplate = TemplateService.getEquipmentListTemplate();
    const html = TemplateService.renderEquipmentListTemplate(
      equipmentListTemplate,
      {
        version: equipList.version,
        unitLabel: equipList.unitLabel,
        generatedAt: equipList.generatedAt,
        sections: equipList.sections.map(section => ({
        ...
        totalItemCount: equipList.totalItemCount,
      },
      project,
      client
    );
  ...
  // Export document as PDF (opens in new window for print-to-PDF)
  function handleExportDocumentPDF(fileData: string) {
    const html = decodeURIComponent(escape(atob(fileData)));
    openPDFWindow(html);
  }
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList>
          ...
          <TabsTrigger value="production">
            Production
            {(project.workItems?.filter(w => w.kind === 'production').length || 0) > 0 && (
              <Badge variant="secondary" className="ml-2">
                {project.workItems?.filter(w => w.kind === 'production').length}
              </Badge>
            )}
          </TabsTrigger>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          {/* CE Status Widget (NEW_BUILD only) */}
        ...
        <TabsContent value="quotes" className="space-y-4">
          ...
          {/* Customer Offer Panel - Show for each SENT or ACCEPTED quote */}
          {project.quotes
          ...
          {/* Equipment List Card - Part of quotation process */}
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Non-priced equipment list from configuration (can be included with quotes)
                </CardDescription>
              </div>
              <Button onClick={() => setShowGenerateEquipmentListDialog(true)}>
                <Package className="h-4 w-4 mr-2" />
                Generate
              </Button>
            </CardHeader>
            <CardContent>
              {(project.equipmentLists?.length || 0) === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <div className="w-14 h-14 rounded-xl bg-slate-100 flex items-center justify-center mb-4">
                    <Package className="h-7 w-7 text-slate-400" />
                  </div>
                  <h4 className="text-base font-medium text-slate-900 mb-1">No equipment lists</h4>
                  <p className="text-sm text-slate-500 max-w-xs">
                    Generate an equipment list from the project configuration to include with quotes or share with clients.
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {project.equipmentLists
                    ?.sort((a, b) => new Date(b.generatedAt).getTime() - new Date(a.generatedAt).getTime())
                    .map((equipList) => (
                      <div key={equipList.id} className="p-4 border rounded-lg flex items-center justify-between hover:border-teal-300 transition-colors">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-medium">
                              Equipment List v{equipList.version}
                              {equipList.unitLabel && ` (${equipList.unitLabel})`}
                            </p>
                            <p className="text-sm text-slate-500">
                              {equipList.totalItemCount} items • {equipList.sections.length} categories • Generated {formatDate(equipList.generatedAt)}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => setViewingEquipmentList(equipList)}
                          >
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleExportEquipmentListPDF(equipList)}
              ...
              <div>
                ...
                <CardDescription>
                  {project.bomSnapshots.length > 0
                    ...
                    : project.quotes.length === 0
                      ? 'Create a quote first to generate a BOM'
                      : 'Generate a BOM from a quote'}
                </CardDescription>
              </div>
              {project.configuration.items.length > 0 && project.quotes.length > 0 && (
                <Button onClick={handleOpenGenerateBOMDialog} variant="outline">
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  ...
                  <p className="text-sm text-slate-500 max-w-xs mb-5">
                    {project.configuration.items.length === 0
                      ...
                      : project.quotes.length === 0
                        ? 'Create a quote first. BOMs are always generated from a specific quote to ensure traceability.'
                        : 'Generate a BOM from a quote to see parts breakdown and cost analysis.'}
                  </p>
                  {project.configuration.items.length > 0 && project.quotes.length > 0 && (
                    <Button variant="outline" onClick={handleOpenGenerateBOMDialog}>
                      <RefreshCw className="h-4 w-4 mr-2" />
                      Generate BOM
                    </Button>
                  )}
                    ...
                    return (
                      <div key={bom.id} className="border rounded-lg overflow-hidden">
                          ...
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <div className="flex items-center gap-2 text-sm text-slate-500">
                                <span>{formatDateTime(bom.createdAt)}</span>
                                {bom.sourceQuoteNumber && (
                                  <>
                                    <span>•</span>
                                    <span className="flex items-center gap-1">
                                      <FileText className="h-3 w-3" />
                                      From {bom.sourceQuoteNumber} v{bom.sourceQuoteVersion}
                                    </span>
                                  </>
                                )}
                              </div>
                        ...
                        <div className="p-4">
                                  ...
                                  <TableCell>
                                    <div>
                                      <div className="flex items-center gap-2">
                                        ...
                                        {/* Cost Source Badge */}
                                        {item.costSource === 'LIBRARY' && (
                                          <Badge
                                            className="bg-teal-100 text-teal-700 border-teal-200 text-[10px] px-1.5 py-0"
                                            title={item.librarySupplierCost
                                              ? `Library default from Article: ${formatCurrency(item.librarySupplierCost)}${item.librarySupplierCostNote ? ` (${item.librarySupplierCostNote})` : ''}`
                                              : 'Cost from Article Library'
                                            }
                                          >
                                            Library
                                          </Badge>
                                        ...
                                        {item.costSource === 'ESTIMATED' && (
                                          <Badge
                                            ...
                                            title={`Estimated using: ${Math.round((item.estimationRatio || 0.6) * 100)}% of sell price`}
                                          >
                                            Estimated
                                          </Badge>
                                        )}
                                        {item.costSource === 'OVERRIDE' && (
                                          <Badge
                                            className="bg-blue-100 text-blue-700 border-blue-200 text-[10px] px-1.5 py-0"
                                            title="Overridden manually"
                                          >
                                            Override
                                          </Badge>
                                        )}
                                        {/* Fallback for legacy items without costSource */}
                                        {!item.costSource && item.isEstimated && (
                                      ...
                                      {item.costSource === 'ESTIMATED' && item.sellPrice && (
                                        <p className="text-[10px] text-amber-600">
                                          Based on {Math.round((item.estimationRatio || 0.6) * 100)}% of sell price ({formatCurrency(item.sellPrice)})
                                        </p>
                                      )}
                                      {item.costSource === 'LIBRARY' && item.librarySupplierCost && (
                                        <p className="text-[10px] text-teal-600">
                                          Library default: {formatCurrency(item.librarySupplierCost)}
                                          {item.librarySupplierCostNote && ` - ${item.librarySupplierCostNote}`}
                                        </p>
                                      )}
                                    </div>
                                  </TableCell>
                          ...
                          {/* Cost Source Legend */}
                          <div className="mt-3 pt-3 border-t border-dashed flex flex-wrap items-center gap-4 text-xs text-slate-500">
                            <div className="flex items-center gap-1">
                              <Badge className="bg-teal-100 text-teal-700 border-teal-200 text-[10px] px-1.5 py-0">Library</Badge>
                              <span>From Article supplier cost</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <Badge className="bg-amber-100 text-amber-700 border-amber-200 text-[10px] px-1.5 py-0">Estimated</Badge>
                              <span>{Math.round((bom.costEstimationRatio || 0.6) * 100)}% of sell price</span>
                            </div>
                            <div className="flex items-center gap-1">
                              <Badge className="bg-blue-100 text-blue-700 border-blue-200 text-[10px] px-1.5 py-0">Override</Badge>
        ...
        <TabsContent value="production" className="space-y-6">
          ...
          {/* View Mode Toggle & Task Summary Header */}
            ...
            <div className="flex items-center gap-4">
              ...
              {taskSummary && taskSummary.total > 0 && (
                <div className="flex items-center gap-3 text-sm">
                  <span className="text-slate-500">{taskSummary.total} tasks</span>
                  <span className="text-blue-600">{taskSummary.byStatus.IN_PROGRESS || 0} active</span>
                  <span className="text-green-600">{taskSummary.byStatus.COMPLETED || 0} done</span>
                </div>
              )}
            </div>
            ...
            {taskSummary && taskSummary.total > 0 && (
              <div className="flex items-center gap-4 text-sm">
                <span className="text-slate-500">
                  <span className="font-medium text-amber-600">{taskSummary.totalEstimated}h</span> estimated
                </span>
                <span className="text-slate-500">
                  <span className="font-medium text-teal-600">{taskSummary.totalLogged}h</span> logged
                </span>
              </div>
            )}
          </div>
          ...
          <Card>
            ...
            <CardContent>
              {(() => {
                ...
                return productionTasks.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center">
                  <div className="w-14 h-14 rounded-xl bg-slate-100 flex items-center justify-center mb-4">
                ...
                <Table>
                  ...
                  <TableBody>
                    {productionTasks.map((task) => {
                      ...
                      const isOverBudget = task.estimatedHours && (task.totalLoggedHours || 0) > task.estimatedHours;

                      return (
                        <TableRow key={task.id} className={task.status === 'COMPLETED' ? 'opacity-60' : ''}>
                          <TableCell className="font-mono text-xs text-slate-500">{task.workItemNumber}</TableCell>
                          <TableCell>
                            <div>
                          ...
                          <TableCell className={`text-right text-sm font-medium ${isOverBudget ? 'text-red-600' : ''}`}>
                            {(task.totalLoggedHours || 0) > 0 ? `${task.totalLoggedHours}h` : '-'}
                          </TableCell>
        ...
        {/* Orders Tab - Shop Floor Order List */}
        ...
        <TabsContent value="compliance" className="space-y-4">
          <ComplianceTab project={project} onRefresh={loadProject} dossierDeepLink={dossierDeepLink} />
        </TabsContent>
                            ...
                            {doc.fileData && (
                              <>
                                ...
                                <Button
                                  size="sm"
                                  variant="outline"
                                  onClick={() => handleExportDocumentPDF(doc.fileData!)}
                                  title="Export as PDF (opens print dialog)"
                                >
      ...
      {/* Generate Equipment List Dialog */}
      <GenerateEquipmentListDialog
        open={showGenerateEquipmentListDialog}
        onOpenChange={setShowGenerateEquipmentListDialog}
        project={project}
        onGenerated={loadProject}
      />

      {/* Equipment List Viewer Dialog */}
      <Dialog open={!!viewingEquipmentList} onOpenChange={(open) => !open && setViewingEquipmentList(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader className="flex flex-row items-center justify-between">
            ...
            {viewingEquipmentList && (
              <Button
                size="sm"
                variant="outline"
                onClick={() => handleExportEquipmentListPDF(viewingEquipmentList)}
                title="Export as PDF (opens print dialog)"
              >
                <Printer className="h-4 w-4 mr-1" />
                Export PDF
          ...
          {viewingEquipmentList && (
            <EquipmentListViewer document={viewingEquipmentList} />
          )}
        </DialogContent>
      </Dialog>

      {/* Generate BOM Dialog - Requires Quote Selection */}
      <Dialog open={showGenerateBOMDialog} onOpenChange={setShowGenerateBOMDialog}>
        <DialogContent className="max-w-md">
          ...
          <div className="space-y-4 py-4">
            <p className="text-sm text-slate-600">
              Select the quote to generate the BOM from. The BOM will reflect the configuration
              at the time of the quote for traceability.
            </p>

            <div className="space-y-2">
              <label className="text-sm font-medium">Source Quote</label>
              <Select value={selectedBOMQuoteId} onValueChange={setSelectedBOMQuoteId}>
                <SelectTrigger>
                  ...
                  {project.quotes.map((quote) => (
                    <SelectItem key={quote.id} value={quote.id}>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-slate-500">v{quote.version}</span>
                        <Badge
                          variant="outline"
                          className={
                            quote.status === 'ACCEPTED' ? 'border-green-300 text-green-600' :
                            quote.status === 'SENT' ? 'border-blue-300 text-blue-600' :
                            quote.status === 'DRAFT' ? 'border-amber-300 text-amber-600' :
            ...
            {selectedBOMQuoteId && (() => {
              const selectedQuote = project.quotes.find(q => q.id === selectedBOMQuoteId);
              if (!selectedQuote) return null;
              return (
                <div className="p-3 bg-slate-50 rounded-lg text-sm space-y-1">
                  <div className="flex justify-between">
                    <span className="text-slate-500">Quote Total:</span>
                    <span className="font-medium">{formatCurrency(selectedQuote.totalInclVat)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Items:</span>
                    <span>{selectedQuote.lines.length}</span>
                  </div>
                  <div className="flex justify-between">
          ...
          <div className="flex justify-end gap-2">
            <Button variant="outline" onClick={() => setShowGenerateBOMDialog(false)}>
              Cancel
            </Button>
            <Button onClick={handleGenerateBOM} disabled={!selectedBOMQuoteId}>
              <RefreshCw className="h-4 w-4 mr-2" />
              Generate BOM
            </Button>
          </div>
