...
export function TimesheetsScreen() {
  ...
  // Date range state (replaces period toggle)
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const rangeEntries = await TimesheetService.getByDateRange(startStr, endStr, userId);

      // Calculate daily totals within the range
      ...
      while (current <= dateRange.to) {
        totals[formatDateStr(current)] = 0;
  ...
  }, [dateRange, showAllUsers, canViewAll, user?.id]);
  ...
  const current = new Date(dateRange.from);
  while (current <= dateRange.to) {
    rangeDates.push(formatDate(current));
    current.setDate(current.getDate() + 1);
  }
      ...
      <div className="bg-white border-b px-6 py-3">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <span className="text-sm text-slate-500">Period Total:</span>
            <span className="ml-2 font-bold text-lg text-slate-900">
              {(activeTab === 'entries' ? weekTotal : reportsTotal).toFixed(2)}h
            </span>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-4 py-4">
            ...
            {selectedProjectTasks.length > 0 && (
              <div className="space-y-2">
                ...
                <Select
                  value={toSelectValue(formData.taskId || undefined)}
                  onValueChange={(v) => setFormData((prev) => ({ ...prev, taskId: fromSelectValue(v) ?? '' }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select task (optional)" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value={NONE_VALUE}>No specific task</SelectItem>
