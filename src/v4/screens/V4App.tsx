...
import { Ship, Users, FolderOpen, Settings, Database, User, LayoutDashboard, LogOut, Shield, ChevronDown, Lock, Clock, Hash, CalendarDays, LayoutGrid, Clipboard, ChevronRight, History, Folder, Factory, ShoppingCart, MessageSquare } from 'lucide-react';
...
import { FloorAssistantScreen } from './FloorAssistantScreen';
...
const NAV_VISIBILITY: Record<UserRole, Record<NavScreen, boolean>> = {
  ...
  SALES: {
    ...
    staff: false,
    settings: false,
    'floor-assistant': false, // Sales doesn't need floor assistant
  },
  PRODUCTION: {
    ...
    staff: false,
    settings: false,
    'floor-assistant': true, // Floor assistant is essential for production workers!
  },
};
...
function parseScreenFromHash(): Screen {
  ...
  if (path === '/floor-assistant') {
    return { type: 'floor-assistant' };
  }
...
function V4AppContent() {
  ...
  function navigateToFloorAssistant() {
    updateHash('/floor-assistant');
    setCurrentScreen({ type: 'floor-assistant' });
  }
  ...
  function renderScreenContent() {
    switch (currentScreen.type) {
      ...
      case 'floor-assistant':
        // Available to ADMIN, OFFICE, and PRODUCTION roles
        if (!isNavVisible(userRole || 'PRODUCTION', 'floor-assistant')) {
          return <NoAccessScreen screenName="Floor Assistant" {...noAccessProps} />;
        }
        return <FloorAssistantScreen />;
          ...
          {user && (isNavVisible(user.role, 'shopfloor-board') || isNavVisible(user.role, 'production') || isNavVisible(user.role, 'shop-floor-orders')) && (
            <div className="space-y-1">
              ...
              */}
              ...
              {isNavVisible(user.role, 'floor-assistant') && (
                <NavItem
                  icon={MessageSquare}
                  label="Vloer Assistent"
                  active={currentScreen.type === 'floor-assistant'}
                  onClick={navigateToFloorAssistant}
                />
              )}
