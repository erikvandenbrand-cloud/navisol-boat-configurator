...
import { getCurrentAdapterType } from '@/data/persistence';
...
function ImportExportTab() {
  ...
  return (
    <div className="space-y-6">
      {/* Data Persistence Status Card */}
      <Card className={isSupabaseConnected ? 'border-green-200 bg-green-50/50' : 'border-amber-200 bg-amber-50/50'}>
        <CardHeader className="pb-3">
          <CardTitle className="flex items-center gap-2 text-base">
            <Database className={`h-5 w-5 ${isSupabaseConnected ? 'text-green-600' : 'text-amber-600'}`} />
            Data Persistence Status
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">
                {isSupabaseConnected ? 'Supabase Connected' : 'LocalStorage Mode'}
              </span>
            </div>
            <Badge variant={isSupabaseConnected ? 'default' : 'secondary'} className={isSupabaseConnected ? 'bg-green-600' : ''}>
              {adapterType === 'supabase' ? 'Cloud Storage' : 'Browser Storage'}
            </Badge>
          </div>
          <p className="text-sm text-slate-600 mt-2">
            {isSupabaseConnected
              ? 'Your data is being persisted to Supabase cloud database. Data is synced across devices and sessions.'
              : 'Data is stored in your browser\'s LocalStorage. This data will be lost if you clear browser data or use a different browser/device.'}
          </p>
          {!isSupabaseConnected && (
            <p className="text-sm text-amber-700 mt-2 flex items-center gap-1">
              <AlertTriangle className="h-4 w-4" />
              Configure NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY environment variables to enable cloud storage.
            </p>
          )}
        </CardContent>
      </Card>
...
function UserManagementTab() {
  ...
  const [newUser, setNewUser] = useState({
    email: '',
    name: '',
    role: 'OFFICE' as UserRole, // Default new users to OFFICE (v320)
    password: '',
    department: '',
  });
  ...
  async function handleCreateUser() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        setNewUser({ email: '', name: '', role: 'OFFICE', password: '', department: '' });
      } else {
        alert(result.error);
      }
    } catch (error) {
  ...
  // Roles v320: ADMIN, OFFICE, SALES, PRODUCTION
  const roles: UserRole[] = ['ADMIN', 'OFFICE', 'SALES', 'PRODUCTION'];
                  ...
                  <div className="flex items-center gap-3">
                    <Badge
                      ...
                      className={
                        ...
                        user.role === 'OFFICE' ? 'bg-amber-50 text-amber-700 border-amber-200' :
