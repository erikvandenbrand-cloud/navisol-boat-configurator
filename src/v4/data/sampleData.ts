...
import { LibrarySeedService, WorkInstructionService } from '@/domain/services';

export async function initializeSampleData() {
  const context = getAuditContext();

  // Initialize library taxonomy (idempotent - provides category structure)
  await LibrarySeedService.initializeTaxonomy(context);

  // PRODUCTION MODE: Skip demo/sample data
  if (process.env.NODE_ENV === 'production') {
    console.log('Production mode: Sample data disabled. Library structure initialized.');
    return;
  }

  console.log('Development mode: Initializing sample data...');

  // Development only: Sample articles and work instructions
