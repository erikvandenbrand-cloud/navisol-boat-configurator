...
import { TemplateService, TEMPLATE_PLACEHOLDERS, getPlaceholderDocsForType, type PlaceholderDefinition } from '@/domain/services/TemplateService';
...
export function TemplateEditorDialog({
  ...
  // Get document-type-specific placeholders, falling back to generic ones
  const typeSpecificPlaceholders = template
    ? getPlaceholderDocsForType(template.type)
    : TEMPLATE_PLACEHOLDERS;

  // Use type-specific placeholders if available, otherwise use generic
  const placeholdersToShow = typeSpecificPlaceholders.length > 0
    ? typeSpecificPlaceholders
    : TEMPLATE_PLACEHOLDERS;

  // Group placeholders by category
  const placeholdersByCategory = placeholdersToShow.reduce((acc, p) => {
      ...
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 flex gap-4 overflow-hidden">
          ...
          <div className="w-64 border-r pr-4 overflow-y-auto">
            ...
            {template && typeSpecificPlaceholders.length > 0 && (
              <Badge variant="outline" className="mb-2 text-[10px] border-teal-300 text-teal-700">
                {template.type.replace(/_/g, ' ')}
              </Badge>
            )}
            <p className="text-xs text-slate-500 mb-4">
              Click to insert into template. {placeholdersToShow.length} available.
            </p>
                  ...
                  <CollapsibleContent className="mt-1 space-y-1">
                    {placeholders.map((p) => (
                      <button
                        ...
                        className="w-full text-left p-2 text-xs rounded hover:bg-teal-50 border border-transparent hover:border-teal-200 transition-colors group"
                        title={`${p.description}\nExample: ${p.example}`}
                      >
                        <code className="text-teal-600 font-mono text-[11px]">{p.key}</code>
                        <p className="text-slate-600 mt-0.5 font-medium">{p.label}</p>
                        <p className="text-slate-400 mt-0.5 text-[10px] group-hover:text-slate-500">
                          e.g. {p.example}
                        </p>
                      </button>
                    ))}
