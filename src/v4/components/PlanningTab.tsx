...
import { ProjectRepository, StaffRepository } from '@/data/repositories';
...
export function PlanningTab({ project, onRefresh }: PlanningTabProps) {
  ...
  const [addFromStaffDialogOpen, setAddFromStaffDialogOpen] = useState(false);
  ...
  function handleOpenAddResourceFromStaff() {
    setAddFromStaffDialogOpen(true);
  }

  async function handleAddResourceFromStaff(staffMember: StaffMember) {
    // Check if resource with same name already exists
    const existingResource = resources.find(
      (r) => r.name.toLowerCase() === staffMember.name.toLowerCase()
    );
    if (existingResource) {
      alert(`A resource with name "${staffMember.name}" already exists.`);
      return;
    }

    // Create new resource from staff member (one-time copy, no sync)
    const newResource: PlanningResource = {
      id: generateUUID(),
      name: staffMember.name,
      role: staffMember.label, // Use staff label as role hint
    ...
    setAddFromStaffDialogOpen(false);
    await onRefresh();
  }
  ...
  // Quick-add resource from staff (used in task dialog)
  async function handleQuickAddResourceFromStaff(staffMember: StaffMember): Promise<PlanningResource> {
    // Check if resource with same name already exists
    const existingResource = resources.find(
      (r) => r.name.toLowerCase() === staffMember.name.toLowerCase()
    );
    ...
    // Create new resource from staff member
    const newResource: PlanningResource = {
      id: generateUUID(),
      name: staffMember.name,
      role: staffMember.label,
      notes: staffMember.notes,
    };
    ...
    <div className="space-y-6">
      ...
      <Card data-testid="planning-resources-section">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          ...
          {!isReadOnly && (
            <div className="flex items-center gap-2">
              <Button onClick={handleOpenAddResourceFromStaff} variant="outline" size="sm">
      ...
      <TaskPlanningDialog
        ...
        onQuickAddResource={handleQuickAddResourceFromStaff}
      />
      ...
      {/* Add from Staff Dialog */}
      <AddFromStaffDialog
        open={addFromStaffDialogOpen}
        onOpenChange={setAddFromStaffDialogOpen}
        existingResourceNames={resources.map((r) => r.name.toLowerCase())}
        onAdd={handleAddResourceFromStaff}
      />
...
interface TaskPlanningDialogProps {
  ...
  onQuickAddResource?: (staff: StaffMember) => Promise<PlanningResource>;
}
...
function TaskPlanningDialog({
  ...
  // Local resources list (includes quick-added ones during this session)
  const [localResources, setLocalResources] = useState<PlanningResource[]>(resources);

  // Staff quick-add state
  const [staffList, setStaffList] = useState<StaffMember[]>([]);
  const [isLoadingStaff, setIsLoadingStaff] = useState(false);
  const [showStaffPicker, setShowStaffPicker] = useState(false);

  // Sync local resources with prop when dialog opens
  ...
  if (open !== prevOpen || task !== prevTask) {
    ...
    if (open) {
      ...
      setLocalResources(resources); // Reset to current resources
      // Load staff list for quick-add
      loadStaffList();
    }
  }
  ...
  // Get staff members not yet added as resources (for quick-add)
  const availableStaffForQuickAdd = staffList.filter((s) => {
    const alreadyResource = localResources.some(
      (r) => r.name.toLowerCase() === s.name.toLowerCase()
    );
    return !alreadyResource;
  });

  async function handleQuickAddStaff(staff: StaffMember) {
    if (!onQuickAddResource) return;

    try {
      const newResource = await onQuickAddResource(staff);
      // Add to local resources so it shows immediately
      setLocalResources((prev) => [...prev, newResource]);
      // Auto-select the new resource as assignee
      setSelectedAssignees((prev) => [...prev, newResource.id]);
      setShowStaffPicker(false);
    } catch (error) {
      console.error('Failed to quick-add staff:', error);
    }
  }
          ...
          <div className="space-y-2">
            ...
            {localResources.length === 0 ? (
              <div className="text-sm text-slate-500">
                ...
                {onQuickAddResource && staffList.length > 0 && (
                  <>
                    {' '}
                    <Popover open={showStaffPicker} onOpenChange={setShowStaffPicker}>
                      ...
                      <PopoverContent className="w-64 p-2" align="start">
                        <div className="space-y-2">
                          <p className="text-xs font-medium text-slate-700">Select staff to add:</p>
                          {isLoadingStaff ? (
                            <p className="text-xs text-slate-500">Loading...</p>
                          ) : availableStaffForQuickAdd.length === 0 ? (
                            <p className="text-xs text-slate-500">All staff already added as resources.</p>
                          ) : (
                            <div className="max-h-40 overflow-y-auto space-y-1">
                              {availableStaffForQuickAdd.map((staff) => (
                                <button
                                  key={staff.id}
                                  type="button"
                                  onClick={() => handleQuickAddStaff(staff)}
                                ...
                                >
                                  ...
                                  <div className="min-w-0">
                                    ...
                                    {staff.label && (
                                      <p className="text-xs text-slate-500 truncate">{staff.label}</p>
                                    )}
                                  </div>
                                </button>
                              ))}
                ...
                {!(onQuickAddResource && staffList.length > 0) && ' Add resources first.'}
              </div>
            ) : (
              <div className="space-y-2">
                ...
                {/* Quick add from staff link */}
                {onQuickAddResource && availableStaffForQuickAdd.length > 0 && (
                  <Popover open={showStaffPicker} onOpenChange={setShowStaffPicker}>
                    <PopoverTrigger asChild>
                      <button
                        type="button"
...
interface AddFromStaffDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  existingResourceNames: string[];
  onAdd: (staff: StaffMember) => Promise<void>;
}

function AddFromStaffDialog({
  open,
  onOpenChange,
  existingResourceNames,
  onAdd,
}: AddFromStaffDialogProps) {
  const [staffList, setStaffList] = useState<StaffMember[]>([]);
  ...
  // Filter staff based on search and exclude already-added
  const availableStaff = staffList.filter((s) => {
    // Exclude already added resources (by name, case-insensitive)
    if (existingResourceNames.includes(s.name.toLowerCase())) {
      return false;
    }
    // Apply search filter
    if (searchQuery) {
  ...
  async function handleAdd(staff: StaffMember) {
    setIsAdding(true);
    try {
      await onAdd(staff);
    } finally {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Select a staff member to add as a planning resource. This creates a copy (no sync).
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="py-4">
          ...
          <div className="border rounded-md max-h-64 overflow-y-auto">
            {isLoading ? (
              <div className="p-4 text-center text-slate-500">Loading staff...</div>
            ) : availableStaff.length === 0 ? (
              <div className="p-4 text-center text-slate-500">
                {staffList.length === 0
                  ? 'No staff members defined yet. Go to System → Staff to add staff.'
                  : searchQuery
                    ? 'No matching staff found.'
                    : 'All staff members are already added as resources.'}
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {staff.label && (
                          <p className="text-xs text-slate-500">{staff.label}</p>
                        )}
                      </div>
                    </div>
                    <Button
          ...
          <p className="text-xs text-slate-500 mt-3">
            Resources are copied from staff. Changes to staff list won't affect existing resources.
          </p>
        </div>
