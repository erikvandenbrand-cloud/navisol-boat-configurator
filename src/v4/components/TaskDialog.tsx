...
interface TaskDialogProps {
  ...
  task: WorkItem | null; // null = create new
...
export function TaskDialog({
  ...
  // Load work instructions once when dialog opens
  useEffect(() => {
    if (open) {
      WorkInstructionService.getPublished().then(setWorkInstructions);
    }
  }, [open]);
  ...
  useEffect(() => {
    if (task) {
      ...
      setAssignedTo(getPrimaryAssignee(task) || '');
      setNotes(task.notes || '');
      setWorkInstructionId(task.workInstructionId || '');
    } else {
    ...
    try {
      if (isEdit && onUpdate) {
        await onUpdate({
          ...
          assigneeIds: assignedTo.trim() ? [assignedTo.trim()] : [],
          notes: notes.trim() || undefined,
          workInstructionId: workInstructionId || undefined,
        });
      } else {
        await onSave({
          title: title.trim(),
          ...
          // Note: CreateTaskInput doesn't have workInstructionId yet
          // The caller should use CreateWorkItemInput instead
        });
      }
      onOpenChange(false);
    } catch (error) {
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {isEdit ? `Edit Task #${task.workItemNumber}` : 'Create New Task'}
          </DialogTitle>
          <DialogDescription>
            {isEdit
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 overflow-hidden flex flex-col">
            ...
            <TabsContent value="details" className="flex-1 overflow-y-auto space-y-4 py-4">
              <TaskFormFields
                ...
                setWorkInstructionId={setWorkInstructionId}
                ...
                <div className="bg-teal-50 p-4 rounded-lg">
                  ...
                  {task.estimatedHours && (
                    <div className="flex justify-between items-center mt-2 text-sm">
                      ...
                      <span className={(task.totalLoggedHours || 0) > task.estimatedHours ? 'text-red-600' : 'text-green-600'}>
                        {(task.totalLoggedHours || 0) > task.estimatedHours ? '+' : ''}
                        {(task.totalLoggedHours || 0) - task.estimatedHours}h
                      </span>
                    </div>
                  )}
                </div>
...
function TaskFormFields({
  ...
  setWorkInstructionId: (v: string) => void;
    ...
    <>
      ...
      {workInstructions.length > 0 && (
        <div>
          ...
          <Select
            value={workInstructionId || '__none__'}
            onValueChange={(v) => setWorkInstructionId(v === '__none__' ? '' : v)}
          >
            <SelectTrigger className="mt-1">
              <SelectValue placeholder="Link to work instruction (optional)" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="__none__">No instruction linked</SelectItem>
          ...
          <p className="text-xs text-slate-500 mt-1">
            Optional. Link to a published work instruction for reference.
          </p>
        </div>
      )}
