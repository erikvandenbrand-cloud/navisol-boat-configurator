...
import { LibraryArticleService, LibraryCategoryService, LibrarySubcategoryService, SupplierService } from '@/domain/services';
import type { Supplier } from '@/domain/models/order-item';
...
export function ArticleDialog({
  ...
  // Article-level supplier cost (for BOM defaults)
  const [supplierCostExclVat, setSupplierCostExclVat] = useState('');
  const [supplierCostSourceNote, setSupplierCostSourceNote] = useState('');
  ...
  useEffect(() => {
    if (open) {
      if (mode === 'create') {
        ...
        setSupplierId(article.supplierId || '');
        ...
        // Article-level supplier cost
        setSupplierCostExclVat(article.supplierCostExclVat?.toString() || '');
        setSupplierCostSourceNote(article.supplierCostSourceNote || '');
  ...
  async function loadCategoriesAndSubcategories() {
    try {
      const [cats, subs, supps] = await Promise.all([
  ...
  function resetForm() {
    ...
    setLeadTimeDays('');
    setNotes('');
    setSupplierCostExclVat('');
    setSupplierCostSourceNote('');
    setAttachments([]);
    setError(null);
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (mode === 'create') {
        ...
        const supplierCostNum = supplierCostExclVat ? parseFloat(supplierCostExclVat) : undefined;
        ...
        const input: CreateArticleInput = {
          ...
          supplierCostExclVat: supplierCostNum || null,
          supplierCostSourceNote: supplierCostSourceNote || null,
        };
        ...
        // Update supplier cost on the article (if changed)
        const supplierCostNum = supplierCostExclVat ? parseFloat(supplierCostExclVat) : null;
        const currentSupplierCost = article.supplierCostExclVat ?? null;
        const currentSourceNote = article.supplierCostSourceNote ?? null;
        const newSourceNote = supplierCostSourceNote || null;

        if (supplierCostNum !== currentSupplierCost || newSourceNote !== currentSourceNote) {
          await LibraryArticleService.updateSupplierCost(
            article.id,
            supplierCostNum,
            newSourceNote,
            context
        ...
        // Update supplier on the article (if changed)
        const selectedSupplier = suppliers.find(s => s.id === supplierId);
        if (supplierId !== (article.supplierId || '')) {
          await LibraryArticleService.update(
            article.id,
            {
        ...
        <div className="space-y-4 py-4">
          ...
          {mode !== 'create' && article && (
            <div className="space-y-3">
              ...
              {/* Supplier - editable when editing */}
              <div>
          ...
          {/* Default Supplier Cost (for BOM) - editable in create and edit modes */}
          <div className="border-t pt-4">
            <h4 className="text-sm font-medium text-slate-700 mb-1">Default Supplier Cost (optional)</h4>
            <p className="text-xs text-slate-500 mb-3">
              If set, new BOM lines will use this as default cost instead of estimating from sell price.
              {mode !== 'create' && ' Existing BOM lines are not affected.'}
            </p>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="supplierCostExclVat">Supplier Cost (€ excl. VAT)</Label>
                <Input
                  id="supplierCostExclVat"
                  type="number"
                  step="0.01"
                  value={supplierCostExclVat}
                  onChange={(e) => setSupplierCostExclVat(e.target.value)}
                  placeholder="Optional"
                />
              </div>
              <div>
                <Label htmlFor="supplierCostSourceNote">Source Note</Label>
                <Input
                  id="supplierCostSourceNote"
                  value={supplierCostSourceNote}
                  onChange={(e) => setSupplierCostSourceNote(e.target.value)}
                  placeholder="e.g., Supplier X price list 2026-01"
                />
              </div>
            </div>
          </div>
