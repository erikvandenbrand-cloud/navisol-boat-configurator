...
export function ProjectContextPanel({ project, onNavigateToTab }: ProjectContextPanelProps) {
  ...
  // All work items
  const workItems = useMemo(() => project.workItems || [], [project.workItems]);

  // Planning resources
  const planningResources = useMemo(() => project.planning?.resources || [], [project.planning]);

  // Planning tasks (work items with kind: 'planning')
  const planningTasks = useMemo(
    () => workItems.filter((wi) => wi.kind === 'planning'),
    [workItems]
  );

  // Production tasks (work items with kind: 'production')
  const productionTasks = useMemo(
    () => workItems.filter((wi) => wi.kind === 'production'),
    [workItems]
  );
  ...
  // Production tasks with dependencies (has dependsOnIds)
  const linkedTasks = useMemo(() => {
    return productionTasks.filter((t) => t.dependsOnIds && t.dependsOnIds.length > 0);
  }, [productionTasks]);

  // Production tasks without dependencies
  const unlinkedProductionTasks = useMemo(() => {
    return productionTasks.filter((t) => !t.dependsOnIds || t.dependsOnIds.length === 0);
  }, [productionTasks]);

  // Assigned production tasks (has at least one assignee)
  const assignedProductionTasks = useMemo(() => {
    return productionTasks.filter((t) => t.assigneeIds && t.assigneeIds.length > 0);
  }, [productionTasks]);
  ...
  const productionAssignees = useMemo(() => {
    ...
    productionTasks.forEach((t) => {
      if (t.assigneeIds && t.assigneeIds.length > 0) {
        t.assigneeIds.forEach((id) => assignees.add(id));
      }
    });
  ...
  const autoSummary = useMemo(() => {
    ...
    if (planningTasks.length > 0) {
      const doneTasks = planningTasks.filter((t) => t.status === 'COMPLETED').length;
  ...
  const suggestions = useMemo((): Suggestion[] => {
    ...
    const unassignedTasks = productionTasks.filter(
      (t) => (!t.assigneeIds || t.assigneeIds.length === 0) && t.status !== 'COMPLETED' && t.status !== 'CANCELLED'
    );
  ...
  return (
    <div className="space-y-4">
      ...
      {(() => {
        ...
        productionTasks.forEach((t) => {
          if (t.assigneeIds && t.assigneeIds.length > 0) {
            t.assigneeIds.forEach((id) => allStaffNames.add(id));
          }
        });
        ...
        // From planning task assignees (resource IDs)
        ...
        const staffSummary = Array.from(allStaffNames).map((name) => {
          // For production, name is assigneeId (could be userId or staff name)
          const prodTasks = productionTasks.filter((t) => t.assigneeIds && t.assigneeIds.includes(name));
