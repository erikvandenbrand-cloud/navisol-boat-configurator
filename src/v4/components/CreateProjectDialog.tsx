...
const PROJECT_TYPES: { value: ProjectType; label: string; description: string }[] = [
  ...
  { value: 'B2B_AFBOUW', label: 'B2B Afbouw', description: 'Capaciteitsverhuur - afbouw voor derden op basis van klant-ontwerp' },
];
...
export function CreateProjectDialog({ open, onOpenChange, onCreated }: CreateProjectDialogProps) {
    ...
    try {
      ...
      if (selectedModelData) {
        ...
        await ConfigurationService.addItem(project.id, {
          ...
          description: `Base vessel - ${selectedModelData.model.range} range, ${selectedModelData.version.specifications?.dimensions?.lengthOverallM ?? '—'}m x ${selectedModelData.version.specifications?.dimensions?.beamOverallM ?? '—'}m`,
          quantity: 1,
          unit: 'pcs',
        ...
        // Prefill vesselIdentity from model data (one-time initialization)
        // GOVERNANCE (Patch 5): Only project-owned fields are stored in vesselIdentity.
        // Model facts (LOA, Beam, etc.) are resolved from BoatModel at read-time.
        if (projectType === 'NEW_BUILD') {
          await ProjectRepository.update(project.id, {
            vesselIdentity: {
              modelName: selectedModelData.model.name,
              // NOTE: loaMeters, beamMeters, etc. are NOT set here anymore.
              // Model facts are owned by BoatModel and resolved at read-time.
              initFromModel: {
                boatModelId: selectedModelData.version.id,
                boatModelName: selectedModelData.model.name,
  ...
  function canProceed(): boolean {
    switch (step) {
      case 'type':
        return !!projectType;
      case 'model':
        // Model selection is optional for REFIT/MAINTENANCE/B2B_AFBOUW
  ...
  function nextStep() {
    if (step === 'type') {
      // Skip model selection for REFIT, MAINTENANCE, and B2B_AFBOUW projects
      if (projectType === 'NEW_BUILD') {
        setStep('model');
      } else {
        setStep('client');
  ...
  const steps = projectType === 'NEW_BUILD'
    ...
    : ['type', 'client', 'details']; // REFIT, MAINTENANCE, B2B_AFBOUW use 3 steps
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-slate-500">
                            {version.specifications?.dimensions?.lengthOverallM ?? '—'}m x {version.specifications?.dimensions?.beamOverallM ?? '—'}m • {model.range}
                          </p>
                      ...
                      <div className="text-right">
                        ...
                        <Badge variant="outline" className="text-xs">
                          CE Cat {version.specifications?.compliance?.designCategory || '—'}
                        </Badge>
                      </div>
                    </div>
                  </button>
